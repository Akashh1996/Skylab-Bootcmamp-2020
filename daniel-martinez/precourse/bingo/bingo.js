...
function cambiarCarton(nombre){
    switch (prompt(`${nombre}, ¿quieres empezar a jugar o cambiar de cartón? (JUGAR/CAMBIAR)`)){
        case 'JUGAR':
            return nuevoTurno(bingoCard, nombre);
        case 'CAMBIAR':
            bingoCard = generarCarton(bingoCard);
...
function nuevoTurno(bingoCard, nombre){
    ...
    if (hayLinea(bingoCard)){
        ...
        console.log(`Has completado el bingo en ${turnos} turnos, que equivalen a ${100-turnos} puntos.`);
        window.alert(`¡FELICIDADES, HAS COMPLETADO EL BINGO! TU PUNTUACIÓN ES DE ${100-turnos} PUNTOS\nA continuación puedes comprovar tu posición en el ranking.`);
        ...
        if (window.confirm('¿Quieres volver a jugar?')){
            ...
            l = 0;
            return bingo();
        } else { return console.log('¡Hasta otra!')}
    } else {
...
function hayLinea(bingoCard){
    if (bingoCard[0]['matched'] && bingoCard[1]['matched'] && bingoCard[2]['matched'] && bingoCard[3]['matched'] && bingoCard[4]['matched'] && l === 0){
        l = 1;
        return true;
    }
    if (bingoCard[5]['matched'] && bingoCard[6]['matched'] && bingoCard[7]['matched'] && bingoCard[8]['matched'] && bingoCard[9]['matched'] && l === 0){
        l = 1;
        return true;
    }
    if (bingoCard[10]['matched'] && bingoCard[11]['matched'] && bingoCard[12]['matched'] && bingoCard[13]['matched'] && bingoCard[14]['matched'] && l === 0){
        l = 1;
        return true;
    }
    hayBingo(bingoCard);
...
function hayBingo(bingoCard){
    if (bingoCard[0]['matched'] && bingoCard[1]['matched'] && bingoCard[2]['matched'] && bingoCard[3]['matched'] && bingoCard[4]['matched'] && bingoCard[5]['matched']
    && bingoCard[6]['matched'] && bingoCard[7]['matched'] && bingoCard[8]['matched'] && bingoCard[9]['matched'] && bingoCard[10]['matched'] && bingoCard[11]['matched']
    && bingoCard[12]['matched'] && bingoCard[13]['matched'] && bingoCard[14]['matched']){
        return true;
    }   
}
...
function mostrarRanking(){
    ...
    rankingOrder.sort(function(a, b) {return b[1] - a[1];});
