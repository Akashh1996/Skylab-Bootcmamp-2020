...
[1,'Barcelona','Palma',40.00,0],
[2,'Barcelona','Londres',85.50,1],
[3,'Barcelona','Moscu',120.30,2],
[4,'Barcelona','Paris',49.99,0],
[5,'Barcelona','Atenas',180.00,1],
[6,'Barcelona','Amsterdam',99.99,0],
[7,'Barcelona','Estambul',210.00,1],
[8,'Barcelona','Dubai',350.00,2],
...
function airlinespro(){
    ...
    for (var i = 0; i < vuelos.length; i++){
        ...
        console.log('ID' + vuelos[i][0] + ': Origen: '+ vuelos[i][1]+ '    Destino: '+ vuelos[i][2] + '  -  ' + vuelos[i][3].toFixed(2) + '€, ' + vuelos[i][4] + ' escalas');
    }
    costemedio /= 10;
...
function admin(){
    switch (prompt('¿Qué función desea realizar? (Introduzca el número)\n1. Añadir vuelos\n2. Eliminar vuelos\n3. Salir')){
        case '1':
            if (vuelos.length == 15){
...
function eliminarVuelo(){
    var idDelete = Number(prompt('Introduzca el ID del vuelo que desea eliminar:'));
    if (isNaN(Number(idDelete))){
        ...
        alert('Ha eliminado el vuelo con ID ' + idDelete + ' correctamente.\nA continuación se muestra la lista de vuelos actualizada.');
        console.log('Lista de vuelos actualizada:');
        for (var i = 0; i < vuelos.length; i++){
            console.log('ID' + vuelos[i][0] + ': Origen: '+ vuelos[i][1]+ '    Destino: '+ vuelos[i][2] + '  -  ' + vuelos[i][3].toFixed(2) + '€, ' + vuelos[i][4] + ' escalas');
        }
...
function user(){
    switch (prompt('¿Qué opción desea? (Introduzca el número)\n1. Buscar por precio\n2. Comprar billete\n3. Salir')){
        case '1':
            buscarPorPrecio();
            break;
        case '2':
...
function buscarPorPrecio(){
    ...
    switch(prompt('Elige una opción:\n1. Precio más alto que\n2. Precio más bajo que\n3. Precio igual a\n4. Salir')){
        ...
        case '3':
            ...
            if (busca.length == 0){
                console.log('No hay vuelos con precio igual a ' + precio.toFixed(2) + '€');
            } else {
                for (var j = 0; j < busca.length; j++){
                    console.log('ID' + busca[j][0] + ': Origen: '+ busca[j][1]+ '    Destino: '+ busca[j][2] + '  -  ' + busca[j][3].toFixed(2) + '€, ' + busca[j][4] + ' escalas');
                }
...
function comprarBillete(){
    ...
    if (isNaN(Number(id))){
        ...
        for (var i = 0; i < vuelos.length; i++){
            if (id == vuelos[i][0]){
                console.log('Compra realizada con importe de '+ vuelos[i][3] +'€. Vuelo ID' + id + ' ' + vuelos[i][1] + ' - ' + vuelos[i][2] + '. Gracias por su compra, vuelva pronto.')
                alert('Ha comprado el vuelo con destino a ' + vuelos[i][2] + '. Gracias por su compra, vuelva pronto.');
            }
        }
    }
}
