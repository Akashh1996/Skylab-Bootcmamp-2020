...
function mostrarVuelos() {
    ...
    for (i = 0; i < flights.length; i++) {
        console.log("El vuelo con id: " + flights[i].id + ", origen " + flights[i].from + " y destino " + flights[i].to + " tiene un coste de " + flights[i].cost + "â‚¬.");
    }
...
function iniciADMIN(){
    switch (prompt("Â¿QuÃ© operaciÃ³n desea realizar?, Introduzca la letra de la opciÃ³n a realizar. a) Introducir vuelo, b) Eliminar vuelo, c) Salir")){
        case 'a':
...
function introduceVuelo(){
    if (flights.length < 15) {
        ...
        mostrarVuelos();
    } else alert("Se ha almacenado el nÃºmero mÃ¡ximo de vuelos posible (15 vuelos).");
}

function eliminarVuelo(){
    var idEliminar = prompt("Por favor introduce el ID del vuelo que desea eliminar");
    var idVuelo = false;
    for(var i = 0; i<flights.length && !idVuelo; i++){
        if(flights[i].id == idEliminar){
            idVuelo = true;
            idEliminar = flights.splice(i,1);
            alert("Vuelo eliminado correctamente. A continuaciÃ³n se muestra la lista de vuelos actualizada:");
            mostrarVuelos();
        }
    }
    if(!idVuelo){
...
function iniciUSER(){
    do {
        var order = prompt("Â¿CÃ³mo desear ordenar el precio de los vuelos? Escriba '+', '-' o '='");
        ...
        case '+':
            ...
            for (var i = 0; i < flights.length; i++) {
                if (flights[i].scale === true) {
                    console.log(`El vuelo con id: ${flights[i].id} to: ${flights[i].to} from: ${flights[i].from} y tiene un coste de ${flights[i].cost} â‚¬. Este vuelo realiza al menos una escala `);
                } else {
                    console.log(`El vuelo con id: ${flights[i].id} to: ${flights[i].to} from: ${flights[i].from} y tiene un coste de ${flights[i].cost} â‚¬. Este vuelo no realiza ninguna escala. `);
                }
            }
