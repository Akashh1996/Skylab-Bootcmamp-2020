...
function newTurn() {
	...
	for (i = 0; i < 5; i++) {
		...
		if (
			(row1 === 'X,X,X,X,X' || row2 === 'X,X,X,X,X' || row3 === 'X,X,X,X,X') &&
			line === false
		) {
			line = true;
			console.log(
...
function passTurn() {
	console.log(
		'Este juego tiene un sistema de puntuación, a menor números necesarios para completar el bingo mayor será la puntuación final obtenida.\nSe empieza con un total de 100 puntos y por cada turno (a partir del 15), se restará 1 punto.\n¡Buena suerte!'
	);
