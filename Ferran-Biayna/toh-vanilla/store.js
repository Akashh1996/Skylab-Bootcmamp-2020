...
class Store {
	...
	anchorListOfHeroes(array = 0) {
		var newArray;
		array === 0 ? (newArray = _heroes) : (newArray = array);
		var li = '';
		for (var index = 0; index < newArray.length; index++) {
			li += `<li class="demoClass"><a href=./detail.html?id=${newArray[index].id} style="text-decoration:none">${newArray[index].name}</a></li>`;
		}
		return li;
	}
	...
	powerHero(heroId) {
		...
		var li = '';
		for (var index = 0; index < newArray.length; index++) {
			li += `<div id="box"><div class="percent"><svg><circle cx="45" cy="45" r="37"></circle><circle cx="45" cy="45" r="37"></circle></svg><div class="number"><span>${newArray[index][1]}</span></div></div><span>${newArray[index][0]}</span></div>`;
		}
		return li;
	}
