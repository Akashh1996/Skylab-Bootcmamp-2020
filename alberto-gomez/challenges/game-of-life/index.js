...
// FUNCION SETUP FINAL DEL TURNO

function endOfGen(firstGrid, lastGrid) {
	for (i = 0; i < lastGrid.length; i++) {
		for (j = 0; j < lastGrid[i].length; j++) {
			lastGrid[i][j] = 0;
		}
	}
	return firstGrid, lastGrid;
}
...
/* function startGame() {
	nextGen(newGameGrid());
	endOfGen(firstGenGrid, nextGenGrid);
} */

function gameGeneration() {
	initialGrid = nextGen(initialGrid);
	// endOfGen(firstGenGrid,ktGenGrid);
}
