...
for (let i = 0; i < flights.length; i++) {
    if (flights[i].scale === true) {
        console.log(`El vuelo número ${flights[i].id} con destino ${flights[i].to} parte desde ${flights[i].from} y tiene un coste de ${flights[i].cost} euros. Este vuelo realiza al menos una escala `);
    } else {
        console.log(`El vuelo número ${flights[i].id} con destino ${flights[i].to} parte desde ${flights[i].from} y tiene un coste de ${flights[i].cost} euros. Este vuelo no realiza ninguna escala. `);
    }
}
...
console.log(`El precio medio del billete es de ${(averagePrice / flights.length).toFixed(2)} euros.`);
...
console.log(`De los vuelos programados para hoy, sólo ${numScale} hacen al menos una escala.`);
        ...
        switch (addFlights) {
            case 'si':
                if (flights.length < 15) {
                    do {
                        var newFlight_id = Number(prompt("Introduce el ID del nuevo vuelo:"));
                        ...
                        var newFlight_cost = Number(prompt("Introduce el coste del nuevo vuelo:"));
                        var newFlight_scale = Boolean(prompt("Indica número de escalas que realiza el vuelo (en el caso de ser un vuelo directo, deja la casilla vacía):"));
                        var newFlight = {
                            id: newFlight_id,
                            to: newFlight_to,
                            from: newFlight_from,
...
// Poder eliminar vuelos mediante el ID.
        ...
        */
        switch (deleteFlight) {
            case 'si':
                var deletedFlight = Number(prompt("Introduzca la ID del vuelo que desea eliminar:")); // ** No confundir deletedFlight con deleteFlight. **
                var flightCounter = 0;
...
// Buscar por precio (más alto, más bajo o igual)
        do {
            var order = prompt("¿Cómo desear ordenar el precio de los vuelos? Escriba 'mayor', 'menor' o 'igual'");
        ...
        */
        ...
        switch (order) {
            case 'mayor': // ** Cambiado a minusculas. **
                ...
                console.log("Estos son los vuelos con los precios ordenados de mayor a menor:");
                for (let i = 0; i < flights.length; i++) {
                    if (flights[i].scale === true) {
                        console.log(`El vuelo número ${flights[i].id} con destino ${flights[i].to} parte desde ${flights[i].from} y tiene un coste de ${flights[i].cost} euros. Este vuelo realiza al menos una escala `);
                    } else {
            ...
            case 'menor': // ** Cambiado a minusculas. **
                ...
                console.log("Estos son los vuelos con los precios ordenados de menor a mayor:");
                for (let i = 0; i < flights.length; i++) {
                    if (flights[i].scale === true) {
                        console.log(`El vuelo número ${flights[i].id} con destino ${flights[i].to} parte desde ${flights[i].from} y tiene un coste de ${flights[i].cost} euros. Este vuelo realiza al menos una escala `);
                    } else {
