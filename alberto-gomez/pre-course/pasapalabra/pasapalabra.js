...
var questions = [
	...
	},
	{
		...
		answers: [
			...
			{ answ: 'popular', quest: 'CON LA P. Perteneciente o relativo al pueblo' }
		]
	},
	{
		letter: 'q',
...
console.log(
	`Bienvenido ${userName} al juego de PASAPALABRA! Donde si aciertas todas las preguntas del rosco, puedes ganar mucho dinero! \nEmpecemos!`
);
	...
	for (let i = 0; i < questions.length; i++) {
		if (questions[i].status === 0) {
			...
			switch (transformToLowerCase) {
				...
				case 'end':
					console.log(
						`Sentimos que tengas que marcharte ${userName}. Has acertado ${right_Q.length} palabras! \nGracias por jugar con nosotros y esperamos volver a verte pronto!`
					);
					throw new Error('El juego se ha terminado.');
				default:
					...
					questions[i].status = 1;
					alert(
						`NOOO, has fallado! La palabra correcta era ${questions[i].answer}`
					);
					//questions.splice(0, 1);
					break;
			}
...
if (right_Q.length === 26) {
	console.log(
		`Enhorabuena ${userName}, has conseguido responder correctamente a todas las preguntas!`
	);
} else {
	console.log(
