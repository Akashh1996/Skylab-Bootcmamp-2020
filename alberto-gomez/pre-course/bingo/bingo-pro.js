...
console.log(
	`Bienvenido ${username}, el juego del bingo va a empezar en breves instantes!`
);
console.info(
...
do {
	...
	line3 = [];
	newCard();
	var cardGenerate = window.confirm(
		'Desea cambiar los numeros del carton? En caso afirmativo, pulse Aceptar.'
	);
...
function bingoTurn() {
	...
	var checkNum = numbers.slice(
		numbers.indexOf(drawNum),
		numbers.indexOf(drawNum) + 1
	);
...
do {
	let endTurn = confirm('Empieza el siguiente turno.');
	if (endTurn == false) {
...
console.log(
	`BINGO! El carton se ha terminado despues de ${turnCounter} turnos.`
);
...
console.log(
	`Muchas gracias por jugar con nosotros ${username} y esperamos verte pronto de nuevo!`
);
