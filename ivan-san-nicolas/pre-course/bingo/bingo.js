function bingo() {
  ...
  function showBoard() {
    ...
    if (foundNums[0] !== undefined && foundNums[0] !== '') {     //If foundNums isn't empty, show the nums found so far
      for (var i = 0; i < foundNums.length; i++) {
  ...
  function checkX() {
    if (player.line === false) {
      for (var i = 0; i < player.board.length; i++) {    //Check for lines
  ...
  function rankingFactory(name, points, turns, line, bingo) {
    return {
      name: name,
      points: points,
      turns: turns,
  ...
  function initRanking(){
    for (var i = 0; i < 9; i++) {   //Initialize ranking
  ...
  function initBoard(){
    for (var i = 0; i < 3; i++) {     //Initialize the board
      for (var k = 0; k < 5; k++) {
