...
const wellcome = () => {
	...
	for (let i = 0; i < flightsDisplay.length; i++) {
		if (flightsDisplay[i].scale === false) {
			console.log(
				'\t- El vuelo con origen ' +
					`%c${flightsDisplay[i].from}` +
					' %cy destino ' +
					`%c${flightsDisplay[i].to}` +
					' %ctiene un coste de ' +
					`%c${flightsDisplay[i].cost + '€'}` +
	...
	console.log(
		`\nEste es el precio de los vuelos ${userNameCap} %c${
			(averageCost / flightsDisplay.length).toFixed(2) + '€'
...
//*************************************MENU PRINCIPAL**************************************** */
const loginName = () => {
	const userLoginName = prompt(
		`¿Desea entrar como: Admin[1] Usuario?[2] Salir[3]?`
	);
	switch (userLoginName) {
		case '1':
			let checkAnswer = false;
			do {
				const addOrDelFly = prompt(
					`Añadir un nuevo vuelo[1] \nEliminar vuelo[2] \nVolver[3]`
				);
				checkAnswer = false;
				switch (addOrDelFly) {
					case '1':
...
const addNewFly = () => {
	...
	if (flightsDisplay.length == 16) {
		...
		const confirmIdValue = confirm(
			`Desea crear el nuevo Vuelo con ID: ${newIdValue}`
		);
	...
	const myNewFly = new getNewFly(...passNewFlyValues);
...
const delFlyValue = () => {
	...
	if (delRequest == true) {
		console.log(
			'\n%c¿Cual de los siguientes vuelos desea eliminar?',
			'font-weight: bold; font-size: 13px'
		);
		for (let i = 0; i < flightsDisplay.length; i++) {
			console.log(
				`Vuelo con id: ${flightsDisplay[i].id}, Destino: ${flightsDisplay[i].to}, Origen: ${flightsDisplay[i].from}`
			);
		}
		let e = true;
		do {
			const idToDelete = parseFloat(
				prompt('Escriba la id que desea eliminar', 0)
			);
			if (isNaN(idToDelete)) {
...
const displayUser = () => {
	...
	switch (userOption) {
		case 1:
			...
			flightsDisplay.forEach((flightDisplay) =>
				console.log(
					`Vuelo con ID: ${flightDisplay.id} Origen: ${flightDisplay.from} Destino: ${flightDisplay.to} Coste: ${flightDisplay.cost}€ Escala: ${flightDisplay.scale}`
				)
			);
			break;
		case 2:
	...
	do {
		...
		idToBuyExist === true
			? console.log(
					`Gracias por comprar el Vuelo con ID: ${buyId}, vuelva pronto!`
			  )
