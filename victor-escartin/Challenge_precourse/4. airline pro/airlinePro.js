...
function admin() {
	var option = prompt(
		'¿Quiere usted INTRODUCIR vuelos, ELIMINAR vuelos, o SALIR?'
	);
	...
	if (option === null) {
		...
		switch (option) {
			...
			default:
				alert(
					'Por favor, vuelva a indicar si usted quiere INTRODUCIR, ELIMINAR o SALIR (en mayúsculas)'
				);
				admin();
		}
	}
...
function introFlight() {
	function newFlight() {
		...
		newF.scale = confirm(
			'Si el vuelo tiene escalas,marque ACEPTAR,de lo contrario marque CANCELAR'
		);
	...
	if (flights.length < 15) {
		...
		console.log(
			'%cEste es listado actual de vuelos disponibles',
			'color: blue'
		);
		for (let i = 0; i < flights.length; i++) {
			if (flights[i].scale === false) {
		...
		for (let i = 0; i < flights.length; i++) {
			...
			alert(
				'Atención,ya no podrá introducir nuevos vuelos si no elimina antes otros'
			);
			admin();
		}
	} else {
		alert(
			'Ha llegado al límite máximo de vuelos, debe eliminar vuelos antes de introducir nuevos'
		);
		admin();
	}
}
...
function deleteFlight() {
	...
	if (!idToDel) {
		...
		if (flights.find(delFlight).scale === false) {
			console.log(
				'El vuelo con ID:' +
					flights.find(delFlight).id +
					' con salida desde ' +
					flights.find(delFlight).from +
					' y destino ' +
					flights.find(delFlight).to +
					' tiene un coste de ' +
					flights.find(delFlight).cost +
					'.00€ y no realiza escala'
			);
		} else {
			console.log(
...
function user() {
	...
	console.log(
		'%cEstos son los vuelos localizados según sus preferencias ordenados por precio:',
		'color:blue'
	);

	for (let k = 0; k < searchFlights.length; k++) {
		console.log(
			'ID:' +
				searchFlights[k].id +
				'_ SALIDA: ' +
				searchFlights[k].from +
				' - DESTINO: ' +
				searchFlights[k].to +
				'; PRECIO:' +
				searchFlights[k].cost +
				'€; ESCALAS: ' +
				searchFlights[k].scale
		); //vuelos menores del precio indicado por el usuario
	}
	...
	if (searchId == null || searchId != Number) {
		...
		if (searchFlights.find(buyFlight) == undefined) {
			alert(
				'El vuelo indicado no existe o supera el precio solicitado, introduce un ID válido'
			);
			user();
		} else {
			console.log('%cHas contratado el siguiente vuelo:', 'color:blue');
			console.log(
				searchFlights.find(buyFlight).id +
					'_ SALIDA: ' +
					searchFlights.find(buyFlight).from +
					' - DESTINO: ' +
					searchFlights.find(buyFlight).to +
					'; PRECIO:' +
					searchFlights.find(buyFlight).cost +
					'€; ESCALAS: ' +
					searchFlights.find(buyFlight).scale
			);
