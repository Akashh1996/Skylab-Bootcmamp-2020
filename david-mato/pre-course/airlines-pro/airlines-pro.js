...
tiene un coste de ${flight.cost} € y no realiza ninguna escala.`);
		...
		if (
			flights.indexOf(flight) >= flights.length - 5 &&
			flights.indexOf(flight) < flights.length - 2
		) {
	...
	console.log(
		`\nEl coste medio de los vuelos es de ${
			Math.round((totalCost / flights.length) * 100) / 100
		} €.`
	);
...
function deleteFlight() {
	...
	for (const flight of flights) {
		flightIDs += `\t· Vuelo ${flight.from} - ${flight.to}. ID: ${(
			'0' + flight.id
		).slice(-2)}.\n`;
	}
	...
	do {
		...
		var correctID = flights.find((obj) => {
...
function addFlights() {
	if (flights.length >= 15) {
		...
		do {
			...
			objectFlight.to === null ||
			objectFlight.to === '' ||
			!isNaN(Number(objectFlight.to))
		);
		do {
			...
			objectFlight.from === null ||
			objectFlight.from === '' ||
			!isNaN(Number(objectFlight.from))
		);
		do {
		...
		do {
			...
			if (objectFlight.scale !== null) {
				...
				objectFlight.scale = objectFlight.scale
					.normalize('NFD')
					.replace(/[\u0300-\u036f]/g, '');
			}
	...
	setTimeout(function () {
		...
		switch (adminOrUserLowerCase) {
			...
			case 'admin':
				do {
					do {
						var adminOption = prompt(
							'¿Qué deseas hacer? Introduce una de las opciones:\n\n1. Crear un vuelo.\n2. Eliminar un vuelo.\n3. Salir.\n'
						);
					...
					switch (+adminOption) {
						case 1:
							...
							if (objectFlight1) {
								do {
									var addFlight = confirm(
										`Has creado el vuelo ${objectFlight1.from} - ${
											objectFlight1.to
										}. Precio: ${objectFlight1.cost} €. ${
											objectFlight1.scale ? 'Con escala' : 'Sin escala'
										}. ID: ${('0' + objectFlight1.id).slice(
											-2
						...
						case 2:
							...
							do {
								var deleteFlight1 = confirm(
									`Has eliminado el vuelo ${deletedFlight[0].from} - ${
										deletedFlight[0].to
									}, con ID: ${('0' + deletedFlight[0].id).slice(
										-2
			...
			case 'usuario':
				...
				do {
					userName
						? (referencePrice = prompt(
								`${userName}, introduce un precio de referencia:`
						  ))
						: (referencePrice = prompt('Introduce un precio de referencia:'));
				...
				function tee(a, fn) {
					var non_matches = [];
					var matches = a.filter(function (e, i, b) {
						var match = fn(e, i, b);
				...
				var sortedByPrice = flights.sort((a, b) =>
					a.cost > b.cost ? 1 : a.cost === b.cost ? (a.id > b.id ? 1 : -1) : -1
				);
				...
				for (const flight of sortedByPrice) {
					if (flight.cost == referencePrice) {
						samePrice += `\t· ${flight.from} - ${flight.to}. Precio: ${
							flight.cost
						} €. ID: ${('0' + flight.id).slice(-2)}.\n `;
					} else if (flight.cost < referencePrice) {
						lowerPrice += `\t· ${flight.from} - ${flight.to}. Precio: ${
							flight.cost
						} €. ID: ${('0' + flight.id).slice(-2)}.\n `;
					} else if (flight.cost > referencePrice) {
						higherPrice += `\t· ${flight.from} - ${flight.to}. Precio: ${
							flight.cost
						} €. ID: ${('0' + flight.id).slice(-2)}.\n `;
					}
				}
				...
				do {
					var buyingFlight = prompt(
						'Por favor, introduce el ID del vuelo que deseas comprar:'
					);
					var correctID = flights.find((obj) => {
						return obj.id == buyingFlight;
					});
				...
				if (buyingFlight === null) {
					...
					console.log(
						`%cHas comprado el vuelo ${correctID.from} - ${
							correctID.to
						}, con ID: ${('0' + correctID.id).slice(-2)}, por ${
							correctID.cost
						} €.`,
						'font-weight: bold; font-size: 12px'
					);
					userName
						? alert(`Gracias por tu compra. ¡Hasta pronto, ${userName}!`)
