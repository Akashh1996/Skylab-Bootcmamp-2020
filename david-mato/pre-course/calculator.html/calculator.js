...
const whiteSpace = [
	buttons['addi'].innerHTML,
	buttons['subt'].innerHTML,
	buttons['mult'].innerHTML,
				...
				if (event.key === buttons['point'].innerHTML) {
					if (input.innerHTML[input.innerHTML.length - 1] !== ')') {
						...
						if (lastParenth > lastSpace) {
							if (
								!input.innerHTML
									.slice(lastParenth + 1, input.innerHTML.length)
									.includes('.')
							) {
								input.innerHTML += event.key;
							}
							...
							if (
								!input.innerHTML
									.slice(lastSpace + 1, input.innerHTML.length)
									.includes('.')
							) {
								input.innerHTML += event.key;
							}
		...
		if (
			...
			if (whiteSpace.includes(event.target.innerHTML)) {
				if (
					...
					input.innerHTML =
						input.innerHTML.slice(0, -3) + ` ${event.target.innerHTML} `;
				}
			} else {
...
const deleteOne = () => {
	...
	var timeoutId = setTimeout(
		() => (stop ? clearTimeout(timeoutId) : deleteMore()),
		600
	);
};
	...
	if (
		...
		const calculate = () => {
			...
			while (arrayOfCalc.length > 1) {
				let findSymbol = arrayOfCalc.find(
					(element) => element === 'x' || element === '÷'
				);
				findSymbol = findSymbol
					? findSymbol
					: arrayOfCalc.find((element) => element === '+' || element === '–');
				var symbolIndex = arrayOfCalc.indexOf(findSymbol);

				switch (findSymbol) {
					case '+':
						result =
							Number(arrayOfCalc[symbolIndex - 1]) +
							Number(arrayOfCalc[symbolIndex + 1]);
		...
		while (input.innerHTML.includes('(')) {
			...
			parenthesis = false;
			if (
				input.innerHTML[indexOfParenth1 - 1] === ' ' ||
				input.innerHTML[indexOfParenth1 - 1] === '(' ||
				indexOfParenth1 === 0 ||
				input.innerHTML[indexOfParenth1 - 1] === '-'
			) {
				if (indexOfParenth1 === 0) {
					input.innerHTML =
						result +
						input.innerHTML.slice(indexOfParenth2 + 1, input.innerHTML.length);
				} else {
					input.innerHTML =
						input.innerHTML.slice(0, indexOfParenth1) +
						result +
						input.innerHTML.slice(indexOfParenth2 + 1, input.innerHTML.length);
				}
			} else if (
				!isNaN(Number(input.innerHTML[indexOfParenth1 - 1])) ||
				input.innerHTML[indexOfParenth1 - 1] === ')' ||
				input.innerHTML[indexOfParenth1 - 1] === '.'
			) {
				input.innerHTML =
