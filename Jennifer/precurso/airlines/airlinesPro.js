...
function mostrarVuelos(flights) {
	for (var i = 0; i < flights.length; i++) {
		let frase =
			...
			flights[i].id +
			' origen ' +
			flights[i].from.toLocaleUpperCase() +
			' y destino ' +
			flights[i].to.toLocaleUpperCase() +
			' tiene un coste de ' +
			flights[i].cost +
			'€.';
		if (flights[i].scale) {
...
function datosVuelos() {
	console.log(
		'El coste medio de los vuelos es de ' +
			(costeMedio / flights.length).toFixed(0) +
			'€.'
	);
...
function añadirVuelo() {
	if (flights.length === 15) {
		...
		alert(
			"Vuelo añadido con éxito, para visualizar los cambios selecciona la opción '3' del menú."
		);
	}
}

function eliminarVuelo() {
	let VueloEliminado = prompt(
	...
	for (var i = 0; i < flights.length && !idEncontrado; i++) {
		if (flights[i].id == VueloEliminado) {
			idEncontrado = true;
			let confirmación = confirm(
				'Deseas borrar el vuelo con origen ' +
					flights[i].from +
					', destino ' +
					flights[i].to +
					' y con un coste' +
			...
			if (confirmación) {
				...
				alert(
					"Vuelo eliminado correctamente, para visualizar los cambios selecciona la opción '3' del menú."
				);
			}
		}
	}
...
function busquedaPrecio(buscarPor) {
	...
	switch (buscarPor) {
		case '1':
			precio = parseFloat(
				prompt('Introduce el precio minimo para la búsqueda')
			);
			while (isNaN(precio)) {
				precio = parseFloat(
					prompt(
						'Introduce el precio minimo para la búsqueda\n' +
		...
		case '2':
			precio = parseFloat(
				prompt('Introduce el precio máximo para realizar tu búsqueda')
			);
			while (isNaN(precio)) {
				precio = parseFloat(
					prompt(
						'Introduce el precio máximo para la búsqueda\n' +
							'(Solo se pueden introducir números)'
					)
				);
			}
		...
		case '3':
			precio = parseFloat(
				prompt('Introduce el precio deseado para buscar vuelos')
			);
			while (isNaN(precio)) {
				precio = parseFloat(
					prompt(
	...
	if (resultadobusqueda.length === 0) {
		console.log(
			'Lo sentimos, no se han encontrado resultados con precio ' +
				fraseResultado +
				precio +
				'€'
		);
		...
		console.log(
			'\n' +
				'Estos son los vuelos con un precio ' +
				fraseResultado +
				precio +
				'€'
		);
