...
while (reiniciarPartida) {
	...
	mostrarCarton();
	continuar = confirm(
		'Por favor, pulse aceptar para continuar y pasar el turno'
	);
	while (continuar) {
		turnoJugador();
		mostrarCarton();
	...
	reiniciarPartida = confirm(
		'Ha terminado la partida. ¿Desea volver a jugar una nueva partida?'
	);
}
	...
	for (var i = 0; i < cartonJugador1.length; i++) {
		for (var j = 0; j < cartonJugador1[i].length; j++) {
			if (cartonJugador1[i][j].numCarton === numeroTurno) {
				console.log(
					'%cNumero encontrado : ' + numeroTurno,
					'background: #FFAE00;'
				);
