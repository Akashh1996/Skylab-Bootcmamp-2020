...
function menuUser() {
	...
	while (!salir) {
		...
		switch (opcionMenu) {
			...
			default:
				opcionMenu = parseInt(
					prompt('POR FAVOR INTRODUCE UNA RESPUESTA VÁLIDA')
				);
		}
	}
}
...
function crearJugadores() {
	...
	do {
		numJugadores = prompt(
			'Por favor, introduce el número de jugadores para la partida,  (máximo 6)'
		);
	} while (
		isNaN(numJugadores) ||
		numJugadores > 6 ||
	...
	for (var i = 0; i < numJugadores; i++) {
		do {
			var nombreJugador = prompt(
				'Por favor, introduce el nombre de los jugadores(Campo obligatorio)'
			);
...
function turno() {
	...
	while (continuarTurno) {
		...
		for (var i = 0; i < jugadores.length; i++) {
			console.log(
				'%c Es turno de: ' + jugadores[i].nombre,
				'background:' + jugadores[i].color
			);
			...
			for (var j = 0; j < 3; j++) {
				for (var m = 0; m < 5; m++) {
					if (jugadores[i].carton[j][m].numCarton === numeroTurno) {
						console.log(
							'%c NUMERO ENCONTRADO: ' + jugadores[i].carton[j][m].numCarton,
							'background: 0008E7'
						);
			...
			console.log(
				'Tus puntos actualizados: ' + jugadores[i].puntos + ' puntos'
			);
			jugadores[i].turnos++;
		}
	}
...
function cambiarCarton() {
	for (var i = 0; i < jugadores.length; i++) {
		jugadores[i].cambiarCarton = confirm(
			'Hola ' + jugadores[i].nombre + ', este es su cartón, desea cambiarlo?'
		);
		while (jugadores[i].cambiarCarton) {
			...
			listaNumerosAleatorios = [];
			jugadores[i].cambiarCarton = confirm(
				'Desea generar otro cartón diferente?'
			);
		}
	}
}
...
function comprobarBingo(posicionJugador) {
	...
	if (contBingo === 15) {
		console.log(
			'%c *****************ENHORABUENA ' +
				jugadores[posicionJugador].nombre.toUpperCase() +
				', HAS HECHO BINGO!!!!***************** ',
			'color: #E700B3'
		);
		console.log(
			'Has terminado el cartón en ' +
				jugadores[posicionJugador].turnos +
				' turnos!'
		);
...
function ranking() {
	...
	for (var j = 0; j < jugadores.length; j++) {
		if (jugadores[j].linea === true) {
			console.log(
				'%c El jugador ' + jugadores[j].nombre + ' ha hecho línea',
				'background:' + jugadores[j].color
			);
		}
		if (jugadores[j].bingo === true) {
			console.log(
				'%c El jugador ' + jugadores[j].nombre + ' ha hecho bingo',
				'background:' + jugadores[j].color
			);
		}
	}
...
function reglasJuego() {
	alert(
		' REGLAS DE JUEGO/ SISTEMA DE PUNTOS:\n' +
			'\n' +
