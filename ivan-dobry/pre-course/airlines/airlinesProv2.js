...
function flight() {
	...
	if (name == null) {
		...
		console.log(
			'Hay un total de ' + totalScales + ' vuelos que realizan escala.'
		);
...
function userMenu() {
	...
	userAction = prompt(
		'Desea buscar vuelos a un precio: \n1.-Mayor \n2.-Menor \n3.-Igual \n4.-Salir'
	);
			...
			for (let i = 0; i < flightsChosen.length; i++) {
				...
				console.log(
					'El vuelo ' +
						flightsChosen[i].id +
						' con origen: ' +
						flightsChosen[i].from +
						', y destino: ' +
						flightsChosen[i].to +
						' tiene un coste de: ' +
						flightsChosen[i].cost +
						scale
				);
			}
...
function buyingMenu() {
	let buyingAction = prompt(
		'Choose the number of the flight you want to buy or press cancel to exit.'
	);
