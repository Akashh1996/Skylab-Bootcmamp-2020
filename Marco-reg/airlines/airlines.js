...
function showWelcomeMessage(nmbr){
    return 'Bienvenido al aeropuerto del Prat '+ nmbr +' estos son los vuelos disponibles:';
...
function AverageCostFligth(flights){
    let result=0;
...
function ShowFiveLast(flights){
    console.log('Estos serán los 5 últimos vuelos:')
    
    for(i=flights.length-1;i>=flights.length-5;i--){
        console.log( 'El vuelo '+ flights[i].FLT+' con destino '+ flights[i].TO +' pertenece a los últimos vuelos del día '+flights[i].COST);
    }
}
