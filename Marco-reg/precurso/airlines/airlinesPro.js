...
function selectAdminUser() {
	let active = true;
	while (active) {
		let optionSelection = window.prompt(
			'Seleccionar Admin o User: \n1. Admin \n2. User \n3.Exit.'
		);
		if (optionSelection.toUpperCase() === 'ADMIN' || optionSelection == 1) {
			...
			optionSelection.toUpperCase() === 'USER' ||
			optionSelection == 2
		) {
			showWelcomeMessage(name);
			...
			optionSelection.toUpperCase() === 'EXIT' ||
			optionSelection == 3
		) {
			active = exit();
		} else {
...
function showWelcomeMessage(nnm, adminStatus) {
	console.log(
...
function adminMenu() {
	let adminSelection = window.prompt(
		'Selecionar una de las opciones \n 1. add  \n 2. delete  \n 3. list \n 4.Exit'
	);
...
function buyIdFligths() {
	...
	console.log(
		'Has comprado ' +
			flights[IdFlight].FLT +
			' con destino ' +
			flights[IdFlight].TO +
			' es el vuelo con el precio elegido ' +
			flights[IdFlight].COST +
