...
function bingo() {
	...
	let name = null;
	name = window.prompt(
		'Por favor introduzca un nombre para empezar a jugar la Partida'
	);
	...
	if (name == '') {
		...
		console.log(
			'Bienvenido ' + name + ' ahora dara comienzo la partida de bingo'
		);
		menuCarton();
	}
}
function menuCarton() {
	let options = window.prompt(
		'Seleccione entre las las dos opciones. \n 1.Obtener cartón \n 2. Salir'
	);
	if (existingNumbers.length >= 100) {
		...
		let isBingo = false;
		for (let i = 100; i >= 0; i--) {
			let generateNewBall = window.prompt(
				'Desea sacar otra bola: \n 1. Yes \n 2. No'
			);
...
function getNewBall() {
	...
	if (existingNumbers.includes(newBall)) {
		for (let i = 0; i < NUM_LINES; i++) {
			for (let k = 0; k < NUM_NUMBERS; k++) {
				if (board[i][k] == newBall) board[i][k] = 'X';
			}
		}
	}
}
