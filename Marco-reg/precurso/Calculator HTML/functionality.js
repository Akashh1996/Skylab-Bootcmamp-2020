...
function deleteOne(e) {
	...
	display.innerHTML = numberWithCommas(
		currentNum.substring(0, currentNum.length - 1).replaceAll(',', '')
	);
	//alert(currentNum.substring(0,currentNum.length-1).replace(',',''))
	currentNum = currentNum
		.substring(0, currentNum.length - 1)
		.replaceAll(',', '');
}
function equal(e) {
	...
	if (currentOp === '+') {
		display.innerHTML = numberWithCommas(
			Math.round((aft + frw) * 100000) / 100000
		);
		currentNum = aft + frw;
	} else if (currentOp === '-') {
		display.innerHTML = numberWithCommas(
