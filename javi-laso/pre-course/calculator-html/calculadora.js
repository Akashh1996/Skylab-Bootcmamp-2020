...
function operation(operationButton) {
    ...
    if (screen.innerHTML === 'Error: division by zero') {
        ...
        miniscreen.innerHTML = miniscreen.innerHTML.substring(0, miniscreen.innerHTML.length - 1) + symbol;
    } else if (!Number.isNaN(parseInt(lastPressedButton)) && savedNumber !== 0.0) {   
        results();
        actualNumber = screen.innerHTML;
        savedNumber = parseFloat(actualNumber)
        miniscreen.innerHTML = actualNumber + ' ' + symbol;
        screen.innerHTML = '0';
    } else if (lastPressedButton === '=' || !Number.isNaN(parseInt(lastPressedButton))) {
        actualNumber = screen.innerHTML;
        savedNumber = parseFloat(actualNumber)
        miniscreen.innerHTML = actualNumber + ' ' + symbol;
        screen.innerHTML = '0';
...
function results() {
    ...
    miniscreen.innerHTML = savedNumber.toString() + ' ' + symbol + ' ' + actualNumber;
    let result = 0;
