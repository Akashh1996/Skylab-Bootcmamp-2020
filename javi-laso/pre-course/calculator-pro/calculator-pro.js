function calculator() {
	...
	while (isIntroducing) {
		nextNumber = prompt(
			"Introduce another number, if you want to stop introduce 'stop'"
		);
	...
	if (numbers.length === 1) {
		results.push({
			'Operation number': id,
			Number: numbers[0],
			'Square root': parseFloat(Math.sqrt(numbers[0].toFixed(3)))
		});
	} else {
		var sum = 0;
		...
		results.push({
			...
			Division: parseFloat(divide.toFixed(3))
		});
	}
...
while (isOperating) {
	question = window.confirm(
		'Do you want to calculate another batch of numbers?'
	);
