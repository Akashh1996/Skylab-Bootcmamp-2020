...
const circleArea = ({radius}) => { //({radius}) a esta funcion se le pasa el elemento que es un objeto que tiene una propiedad radius.
    return (PI * radius * radius).toFixed(2);
};

circleArea(circle);

const {label, radius} = circle; //destructuring propiedades de un objeto.
console.log(label);
console.log(radius);
