// const superheroes = require('./superHeroData.json');
...
function setListOfHeroes(superheroes) {
    let heroContainer = document.getElementById('list-heroes-container');
    ...
    for(let i = 0; i < superheroes.length; i++) {
        ...
        div.prepend(prep);
        prep.innerText = superheroes[i].id;
        let anchorHeroe = document.createElement('a');
        anchorHeroe.setAttribute('class', 'a-hero');
        div.appendChild(anchorHeroe);
        anchorHeroe.innerHTML = superheroes[i].name;
