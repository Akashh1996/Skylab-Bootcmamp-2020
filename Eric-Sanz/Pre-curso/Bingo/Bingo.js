...
let allUsers = [
	{ name: 'Marco', finalPoints: 125, turns: 80 },
	{ name: 'Lara', finalPoints: 93, turns: 90 },
	{ name: 'Salva', finalPoints: 119, turns: 84 },
	{ name: 'Sergio', finalPoints: 138, turns: 77 },
	{ name: 'Laura', finalPoints: 141, turns: 80 },
	{ name: 'Héctor', finalPoints: 80, turns: 87 },
	{ name: 'Ester', finalPoints: 76, turns: 87 },
	{ name: 'Pedro', finalPoints: 100, turns: 89 },
	{ name: 'Luca', finalPoints: 100, turns: 89 }
];

class rankingUsers {
	constructor(name, finalPoints, turns) {
		this.name = name;
...
function bingo() {
	let userName = prompt('Bienvenido a Skylab Bingo! Cúal es su nombre?');
	if (userName == null) {
		return;
...
·Además, depende de con cuantos turnos finalice el Bingo, se le sumarán más puntos:
	...
	function startBingo() {
		let okCard = prompt(
			'Desea cambiar el cartón? [presiona 1] \no jugar con el actual? [Presiona 2].'
		);
		if (okCard == null) {
			return;
		} else if (okCard == 1) {
	...
	function newTurn() {
        ...
        |          |          |          |          |          |
         ------------------------------------------------------`);
			console.log(
				`         Puntos: ${points}                                 Turno: ${countTurns}    `
			);
        ...
        |          |          |          |          |          |
         ------------------------------------------------------`);
            ...
            - ${allUsers[0].name}: ${allUsers[0].finalPoints} puntos en ${allUsers[0].turns} turnos.
            - ${allUsers[1].name}: ${allUsers[1].finalPoints} puntos en ${allUsers[1].turns} turnos.
            - ${allUsers[2].name}: ${allUsers[2].finalPoints} puntos en ${allUsers[2].turns} turnos.
            - ${allUsers[3].name}: ${allUsers[3].finalPoints} puntos en ${allUsers[3].turns} turnos.
            - ${allUsers[4].name}: ${allUsers[4].finalPoints} puntos en ${allUsers[4].turns} turnos.
            - ${allUsers[5].name}: ${allUsers[5].finalPoints} puntos en ${allUsers[5].turns} turnos.
            - ${allUsers[6].name}: ${allUsers[6].finalPoints} puntos en ${allUsers[6].turns} turnos.
            - ${allUsers[7].name}: ${allUsers[7].finalPoints} puntos en ${allUsers[7].turns} turnos.
            - ${allUsers[8].name}: ${allUsers[8].finalPoints} puntos en ${allUsers[8].turns} turnos.
            - ${allUsers[9].name}: ${allUsers[9].finalPoints} puntos en ${allUsers[9].turns} turnos.`);
