...
function welcome() {
  usuario = prompt('Bienvenido a SkyLab Airlines! Cual es tu nombre de usuario?');
  if (usuario == false) {
...
function flights() {
  ...
  for ( i = 0; i < departures.length; i++) {
    ...
    console.log(`· Vuelo con origen: ${departures[i].from} y destino: ${departures[i].to}, con un coste de ${departures[i].cost}€ y ${infoScale}.`);  
  }

  accCost = (accCost / departures.length).toFixed(2);
  console.log(`El coste medio de los vuelos es ${accCost}€ y ${accFlightsScale} vuelos hacen escala.`);
    
  for ( j = departures.length - 5; j < departures.length; j++) {
    lastFlights.push(departures[j].to);
  }
  console.log(`Los destinos de los últimos vuelos del dia són: ${lastFlights}.`);
  typeUser();
}
...
function addFligth() {
    if (departures.length < 15) {
      ...
      questionDestination = prompt('Destino del Vuelo:');
      ...
      questionOrigin = prompt('Procedencia del vuelo:');
      ...
      questionCost = (prompt('Precio:'));
      if (questionCost === null) {
        return;
      } else if (questionCost == false) {
        alert('Debe introducir un precio. Empecemos de nuevo.');
        addFligth();
      } else if (isNaN(questionCost)) {
        alert('Debe introducir un precio. Empecemos de nuevo.');
        addFligth();
      } else {
        newFligth.cost = parseFloat(questionCost);
      }
      questionScale = prompt('Hace escala? Yes/No');
      ...
      questionAddMore = confirm('Quieres añadir otro vuelo?');
      if (questionAddMore) {
          addFligth();
      } else {
        console.log('---------------------------------------------------------------------------------------------');
          ...
          for ( i = 0; i < departures.length; i++) {
            ...
            console.log(`· ID de vuelo ${departures[i].id}, con origen: ${departures[i].from} y destino: ${departures[i].to}, con un coste de ${departures[i].cost}€ y ${infoScale}.`);  
          }
          typeUser();
      }
    } else {
      alert('Has llegado al máximo de vuelos posible, no se pueden introducir más.');
      admin();
    }
}
...
function deleteFligths() {
  ...
  console.log('Estos són los vuelos que hay en el sistema:');
    ...
    fligthId = prompt('Que vuelo desea eliminar? (mediante ID)');
    if (fligthId == null) {
      stop;
      ...
      newOrderId();
      questionDeleteMore = confirm('Quieres eliminar otro vuelo?');
...
function admin() {
  changeFligths = prompt('Desea añadir o eliminar vuelos?');
...
function user() {
  price = prompt('Introduzca el precio con el que quiere realizar la busqueda.');
  if (price == null) {
    stop;
  } else if (price == false) {
    ...
    options = prompt('Buscar por precio mayor, menor o igual?');
    if (options === null) {
      return;
        ...
        if (priceSearcher == 0) {
          ...
          for ( i = 0; i < priceSearcher.length; i++) {
            if (priceSearcher[i].scale) {
              infoScale = 'escala';
            } else {
              infoScale = 'directo';
            }
          console.log(`· ID de vuelo ${priceSearcher[i].id}, con origen: ${priceSearcher[i].from} y destino: ${priceSearcher[i].to}, con un coste de ${priceSearcher[i].cost}€ y ${infoScale}.`);
          }
          buyFligth();
        }
...
function buyFligth() { 
  ...
  if (buyId == null ) {
    ...
    if (priceSearcher.find(flightBought) == undefined){
      ...
      console.log(`Gracias por comprar el Vuelo con ID: ${priceSearcher.find(flightBought).id}, con destino ${priceSearcher.find(flightBought).to}, procedente de ${priceSearcher.find(flightBought).from}, con un coste ${priceSearcher.find(flightBought).cost}€ y ${infoScale}. \nBuen viaje y hasta pronto ${usuario}`);
      priceSearcher = [];
    }
  }
}
