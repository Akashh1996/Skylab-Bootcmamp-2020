...
function printList() {
    ...
    superHeroList.forEach(function (obj) {
        var li = document.createElement('li');
        var numHero = document.createElement('button');
        numHero.setAttribute("class","listElement");
        numHero.textContent = obj.id + "  " + obj.name;
        numHero.appendChild(li)
        list.appendChild(numHero);
    });
    listheader.appendChild(list);
    document.getElementById("navigationContent").appendChild(listheader)
}

function printDashboard() {
    document.getElementById("navigationContent").innerHTML = ""

    let topHeroes = superHeroList.slice(0, 5);
    var headerTop = document.createElement('lh')
    headerTop.textContent = "Top Heroes\r"
    headerTop.setAttribute("class","titleSection");
    var sectionTop = document.createElement('div')
    sectionTop.setAttribute("class","sectionDiv");
    topHeroes.forEach(function (obj) {
        var blockHero = document.createElement('button')
        blockHero.setAttribute("class","boxHero");        
        blockHero.setAttribute("navTarget",obj.name)
        blockHero.setAttribute("onclick","getDetail(event)")
        blockHero.textContent = obj.name
        sectionTop.appendChild(blockHero)
    })
    headerTop.appendChild(sectionTop)
    document.getElementById("navigationContent").appendChild(headerTop)
}

function getDetail(event) {
    let currentDetail = event.currentTarget.getAttribute("navTarget")
    window.history.pushState('page2', 'Title', './list.html#'+ currentDetail);
